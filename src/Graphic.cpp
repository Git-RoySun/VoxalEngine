...
namespace gm{
	Module::Module(){
		...
		window = std::make_unique<Window>(instance);
	...
	void Module::createVkInstance() {
		volkInitialize();
		if (enableValidationLayers) {//Check for validation layer support
		...
		if (enableValidationLayers)
			VK_CHECK_RESULT(vkCreateDebugUtilsMessengerEXT(instance, &debugCreateInfo, nullptr, &debugMessenger), "Failed to Create Debug Messenger!")
	}
