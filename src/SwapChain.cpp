...
namespace gm {
	...
	VkResult SwapChain::acquireNextImage(uint32_t* imageIndex) {
		vkWaitForFences(device.getVkDevice(), 1, &inFlightFences[currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max());

		VkResult result = vkAcquireNextImageKHR(
			device.getVkDevice(), swapChain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphores[currentFrame], // must be a not signaled semaphore
	...
	void SwapChain::createImageViews() {
		...
		for(size_t i = 0; i < swapChainImages.size(); i++) {
			...
			if(vkCreateImageView(device.getVkDevice(), &viewInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS) {
	...
	void SwapChain::createMsaaResources() {
		...
		for(int i = 0; i < colorImages.size(); i++) {
...

			{
				...
				vkGetImageMemoryRequirements(device.getVkDevice(), colorImages[i], &memRequirements);
				VkPhysicalDeviceMemoryProperties memProperties;
				vkGetPhysicalDeviceMemoryProperties(device.getPhysicalDevice(), &memProperties);

				uint32_t memType = 0;
				for(uint32_t x = 0; x < memProperties.memoryTypeCount; x++) {
					if((memRequirements.memoryTypeBits & (1 << x)) && (memProperties.memoryTypes[x].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) == VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) {
						memType = x;
						break;
					}
				}
	...
	void SwapChain::createDepthResources() {
		...
		for(int i = 0; i < depthImages.size(); i++) {
...

			{
				...
				VkMemoryRequirements memRequirements;
				vkGetImageMemoryRequirements(device.getVkDevice(), depthImages[i], &memRequirements);
				VkPhysicalDeviceMemoryProperties memProperties;
				vkGetPhysicalDeviceMemoryProperties(device.getPhysicalDevice(), &memProperties);
 ...
 .memoryTypeIndex = memType,};
	...
	void SwapChain::createSyncObjects() {
		...
		for(size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			if(vkCreateSemaphore(device.getVkDevice(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS || vkCreateSemaphore(device.getVkDevice(), &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS || vkCreateFence(device.getVkDevice(), &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS) {
