...
namespace gm {
	...
	void Device::Initializer::createLogicalDevice() {
		...
		std::set<uint32_t> queueIndices = {
			...
			device->queues.computeFamily,
			device->queues.presentFamily,
		};
	...
	bool Device::Initializer::isDeviceSuitable(VkPhysicalDevice pDevice) {
		...
		uint32_t extensionCount; vkEnumerateDeviceExtensionProperties(pDevice, nullptr, &extensionCount, nullptr);
	...
	Device::QueueFamily Device::Initializer::findQueues(VkPhysicalDevice pDevice) const{
		...
		uint32_t queueFamilyCount = 0; vkGetPhysicalDeviceQueueFamilyProperties(pDevice, &queueFamilyCount, nullptr);
	...
	Device::SwapChainSupportDetails Device::querySwapChainSupport(VkPhysicalDevice pDevice, VkSurfaceKHR targetSurface) {
		...
		uint32_t formatCount; vkGetPhysicalDeviceSurfaceFormatsKHR(pDevice, targetSurface, &formatCount, nullptr);
		if (formatCount != 0) {
			details.formats.resize(formatCount);
			vkGetPhysicalDeviceSurfaceFormatsKHR(pDevice, targetSurface, &formatCount, details.formats.data());
		}

		uint32_t presentModeCount; vkGetPhysicalDeviceSurfacePresentModesKHR(pDevice, targetSurface, &presentModeCount, nullptr);
	...
	void Device::endInstantCommands(VkCommandBuffer commandBuffer) {
		...
		VkFence fence; vkCreateFence(device, &fenceCreateInfo, NULL, &fence);
		VkQueue computeQueue; vkGetDeviceQueue(device, queues.computeFamily,0, &computeQueue);
